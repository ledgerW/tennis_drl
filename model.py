...
class Critic(nn.Module):
    ...
    def __init__(self, full_state_size, action_size, seed, fc1_units=256, fc2_units=128):
        """Initialize parameters and build model.
        Params
        ======
            state_size (int): Dimension of each state
        ...
        self.seed = torch.manual_seed(seed)
        self.bn_input = nn.BatchNorm1d(full_state_size)
    ...
    def forward(self, full_state, action):
        ...
        #full_state_action = torch.cat((x, action), dim=1)
        #x = self.bn_input(full_state_action)
